...
require(['vs/editor/editor.main'], function () {
    ...
    monaco.editor.defineTheme("MacroTheme", {
       ...
       rules: [   // 定义规则，token名称 ~ style  foreground:字体颜色，fontStyle:字体样式
           {token: 'command', foreground: '#0033FF', fontStyle: "bold"} ,   // blue
           {token: 'logic', foreground: '#9933CC'},   // purple
           {token: 'arith', foreground: '#9933CC'},   // purple
           {token: 'signal', foreground: '#B22222'},   // red
           {token: 'operation', foreground: '#00AA00'},   // green
           {token: 'location', foreground: '#B22222'},   // red
           {token: 'exegesis', foreground: '#909090'},   // grey
       ],
    });
    // monaco.editor.setTheme("MacroTheme");
    ...
    if (!model)	// 否则创建新的model
	    model = monaco.editor.createModel(get_file_content('test.txt'), 'MacroLang', uri);
    ...
    var editor = monaco.editor.create(document.getElementById("MacroEdi"),
	{
		// value: "",
	    // value: getcode(),
        // value: get_file_content("testfile.doc"),
		language: "MacroLang",
        fontFamily: "微软雅黑",
        fontSize: 17,
        lineHeight: 25,
    ...
    // var New = '';
    // function saveEditor(){
    //     Old = monaco.model.getValue();
    // }
    // monaco.editor.onKeyUp(() => {
        // New = monaco.editor.getValue();
        // // 当键盘按下，判断当前编辑器文本与已保存的编辑器文本是否一致
        // if(Old != New){
        //     monaco.editor.trigger('anything','editor.action.formatDocument');
        //     monaco.model.setValue("hh")
        // }
        // editor.setValue("jj")
    // });
    ...
    //     var newValue=monaco.editor.getModel().getValue();
    //     monaco.editor.setValue(newValue);
    // })
    ...
    function get_file_content(file_name){
        // import(javascriptDefaults)
        var fso;
        try {
            fso=new ActiveXObject("Scripting.FileSystemObject");
            var openf1 = fso.text("C:\\Users\\mi\\Desktop\\Editor\\HelloWorld\\macro\\modules\\editor\\test.txt", 1, true);
